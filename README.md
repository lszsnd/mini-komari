...
```bash
curl -fsSL https://raw.githubusercontent.com/lszsnd/mini-komari/main/install.sh | bash -s -- master 6060 mytoken http://主控IP:6060 admin 面板密码
```

参数：

```text
master 6060 mytoken http://主控IP:6060 admin 面板密码
│      │    │       │                 │     └─ 网页登录密码
│      │    │       │                 └─────── 网页登录用户
│      │    │       └───────────────────────── 主控公网访问地址，用于网页生成 Agent 命令
│      │    └───────────────────────────────── 上报密钥，自己改复杂一点
│      └────────────────────────────────────── 主控面板端口
└───────────────────────────────────────────── 安装模式
```
...
输入节点名、分组或 Token 时，页面会自动暂停刷新，避免内容被刷新清空；复制按钮如果遇到浏览器限制，会弹出命令让你手动复制。
...
curl -fsSL https://raw.githubusercontent.com/你的用户名/你的仓库/main/install.sh | bash -s -- master 6060 mytoken http://主控IP:6060
```
...
bash install.sh master 6060 mytoken http://主控IP:6060
```
...
POST /api/delete       删除节点，需要网页登录 Basic Auth
```
...
| `MINI_KOMARI_PUBLIC_URL` | 空 | 主控公网访问地址 |
